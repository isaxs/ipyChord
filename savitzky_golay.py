...
def savitzky_golay( y, window_size, order=4, deriv = 0, rate=1):
...
def sgolay2d ( z, window_size, order, derivative = None ):
    ...
    band = z[-1, -1]
    Z[-half_size:, -half_size:] = band + np.abs( np.flipud( np.fliplr(\
                           z[-half_size - 1:-1, -half_size - 1:-1] ) ) - band )

    # top right corner
    band = Z[half_size, -half_size:]
    Z[:half_size, -half_size:] = band - np.abs( np.flipud( Z[half_size +\
                              1:2 * half_size + 1, -half_size:] ) - band )
    # bottom left corner
